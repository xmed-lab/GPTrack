...
class Eval:
    def __init__(self, args):
        ...
        # pretrain_params = torch.load('/home/jyangcu/Pulmonary_Arterial_Hypertension/results/checkpoints/checkpoint_130.pth', map_location='cpu')
    ...
    def eval(self, args):
        self.RViT.eval()
        record_steps = 0
        all_psnr, all_dice, all_ssim, lv_dice, la_dice, ra_dice, rv_dice = [], [], [], [], [], [], []
        pbar = tqdm(self.valid_loader)
        for step, (vids, vids_org, masks) in enumerate(pbar):
            # if step > 0:
            #     print('Finish, Stop in Step ->', step)
            #     break
            # b, c, t, h, w = vids.size
            # input_vids = vids[args.selected_view[0]]
            input_vids = vids
            hidden = torch.zeros(args.batch_size, (args.image_size[0] // args.patch_size) ** 2, args.latent_dim).to(args.device)
            _, inf_flow_all, neg_inf_flow_all, lag_flow, neg_lag_flow, lag_register, forward_regsiter, backward_regsiter = self.RViT(input_vids.transpose(1,2).to(args.device), hidden, train=False)

            com_input_vids = input_vids.squeeze(0).transpose(1,0)[1:, ...].cpu().mul(255)
            com_lag_register = lag_register.squeeze(0).cpu().mul(255)
            com_forward_regsiter = torch.stack(forward_regsiter, dim=1).squeeze(0).cpu().mul(255)
            ...
            for idx in tqdm(range(len(inf_flow_all))):                
                ...
                inf_flow_plt = self.plot_warpgrid(vids_org[0, :, idx, ...], inf_flow_all[idx][0, ...], interval=8, mark='w', heatmap=False)
                inf_flow_plt.savefig(f'./results/flow_result_eval/inf_flow_heatmap_warp_{idx}.png')
                inf_flow_plt.clf()

                # For Contours evaluation
                # c_mask = masks[:, 0, 0, ...].transpose(1,2)
                # inf_flow_seg_plt, c_mask = self.plot_seg_warpgrid(vids_org[0, :, idx+1, ...], c_mask, masks[:, 0, idx+1, ...], lag_flow[0, :, idx, ...], mark='w')
                ...
                if idx < 1:
                    ...
                    c_mask = c_mask
                inf_flow_seg_plt, c_mask = self.plot_seg_warpgrid(vids_org[0, :, idx+1, ...], c_mask, masks[:, 0, idx+1, ...], inf_flow_all[idx][0, ...], mark='w')
                track_segments.append(c_mask)
                inf_flow_seg_plt.savefig(f'./results/flow_result_eval/inf_flow_seg_warp_{idx}.png')
            ...
            for i in range(4):
                s_gt_segment = torch.where(gt_segments == i+1, 1, 0)
                s_track_segments = torch.where(track_segments == i+1, 1, 0)
                _, i_dice, _, _, _ = self._calculate_overlap_metrics(s_gt_segment, s_track_segments)
                if i == 0:
                    lv_dice.append(i_dice.detach().cpu().numpy())
                elif i == 1:
                    la_dice.append(i_dice.detach().cpu().numpy())
                elif i == 2:
                    ra_dice.append(i_dice.detach().cpu().numpy())
                elif i == 3:
        ...
        print("LA_DICE: Mean:{}, Std:{}".format(np.mean(la_dice), np.std(la_dice)))
        print("RA_DICE: Mean:{}, Std:{}".format(np.mean(ra_dice), np.std(ra_dice)))
    ...
    def plot_warpgrid(self, img, warp, segment_result=None, 
        ...
        if img is not None:
            ...
            if heatmap:
                plt.imshow(img.permute(1, 2, 0).mul(255).detach().cpu().numpy(), cmap='gray', vmin=0, vmax=255)
    ...
    def plot_seg_warpgrid(self, img, mask, mask_tgt, warp, segment_result=None, 
        ...
        if img is not None:
            img = img.transpose(1,2)
            mask = mask.float()
            mask_tgt = mask_tgt.transpose(1,2).float()
            # Get the warpping img
            if wrap_seg:
                seg_warp = torch.nn.functional.grid_sample(mask.unsqueeze(0), warp, align_corners=True, mode='nearest')[0]
                seg_warp_a = torch.where(seg_warp > 0, 83, 0)
                seg_warp_b = torch.where(seg_warp > 0, 253, 0)
                seg_warp_c = torch.where(seg_warp > 0, 254, 0)
                seg_warp_rgb = torch.cat([seg_warp_a, seg_warp_b, seg_warp_c], dim=0)

                seg_mask_a = torch.where(mask_tgt > 0, 253, 0)
                seg_mask_b = torch.where(mask_tgt > 0, 83, 0)
                seg_mask_c = torch.where(mask_tgt > 0, 254, 0)
            ...
            img = img.expand(3,-1,-1).mul(255)
            img = torch.where(seg_warp_rgb > 0, seg_warp_rgb, img)
            img = torch.where(seg_mask > 0, seg_mask, img)
            plt.imshow(img.permute(1, 2, 0).detach().cpu().numpy().astype(np.uint8), cmap='viridis', vmin=0, vmax=255)
            # plt.imshow(seg_warp.permute(1, 2, 0).detach().cpu().numpy(), vmin=0, vmax=255, alpha=0.4)
            # plt.imshow(seg_mask.permute(1, 2, 0).detach().cpu().numpy(), vmin=0, vmax=255, alpha=0.4)
        ...
        # plt.quiver(grids[0][::2, ::2], grids[1][::2, ::2], warp_save[::2, ::2, 0], warp_save[::2, ::2, 1], units='xy', angles='uv', color='r', scale_units='xy', scale=1/2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mask-ratio', type=float, default=0.8, help='The ratio of masking area in an image (default: 0.75)')
    parser.add_argument('--blurring', type=bool, default=True, help='Whether blur the image')
    parser.add_argument('--max_sample_rate', type=int, default=1, help='The maximum sampling rate for the video')
    parser.add_argument('--min_sample_rate', type=int, default=1, help='The minimum sampling rate for the video')
    ...
    parser.add_argument('--enable_GPUs_id', type=list, default=[8], help='The number and order of the enable gpus')
