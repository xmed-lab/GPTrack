...
class VM_diffeo_loss(torch.nn.Module):
    ...
    def _degree_matrix(self, vol_shape):
        ...
        if ndims == 2:
            ...
            D = conv_fn(z.permute(0, 4, 1, 2, 3), filt_tf.permute(4, 3, 2, 0, 1), stride=strides, padding=padding)
    ...
    def prec_loss(self, y_pred):
        ...
        if ndims == 2:
            ...
            y_pred1 = y_pred.permute(0, 2, 3, 4, 1) #Batch, x, y, d, Channel
    ...
    def kl_loss(self, y_true, y_pred):
        ...
        if self.flow_vol_shape is None:
            ...
            if ndims == 2:
                ...
                self.flow_vol_shape = (shape[2], shape[3], shape[4])
        ...
        # sigma terms
        sigma_term = self.prior_lambda * self.D.to(device) * torch.exp(log_sigma) - log_sigma
...
class NCC(torch.nn.Module):
    ...
    """
    def __init__(self, win=9, eps=1e-3, ndims=3):
    ...
    def forward(self, I, J):
        ...
        self.win = [self.win_raw] * self.ndims
        ...
        # prepare conv kernel
        if self.ndims == 2:
            weight = torch.ones((1, 1, self.win_raw, self.win_raw), device=I.device, requires_grad=False)
        elif self.ndims == 3:
            weight = torch.ones((1, 1, self.win_raw, self.win_raw, self.win_raw), device=I.device, requires_grad=False)

        # compute filters
        # compute local sums via convolution
        I_sum = self.conv_fn(I, weight, padding=int(win_size/2))
        J_sum = self.conv_fn(J, weight, padding=int(win_size/2))
        I2_sum = self.conv_fn(I2, weight, padding=int(win_size/2))
        J2_sum = self.conv_fn(J2, weight, padding=int(win_size/2))
        IJ_sum = self.conv_fn(IJ, weight, padding=int(win_size/2))

        # compute cross correlation
        # win_size = np.prod(self.win)
        win_size = torch.from_numpy(np.array([np.prod(self.win)])).float()
        win_size = win_size.to(device)
        u_I = I_sum/win_size
        u_J = J_sum/win_size
