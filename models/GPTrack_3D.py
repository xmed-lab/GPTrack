...
import random
from models.loss_3d import VM_diffeo_loss, NCC
...
class DiffeomorphicTransform(nn.Module):
    ...
    def forward(self, velocity):
        vectors = [torch.arange(0, s) for s in velocity.shape[2:]]
        grids = torch.meshgrid(vectors)
        grid = torch.stack(grids)  # y, x, z
        grid = torch.unsqueeze(grid, 0)  # add batch
        grid = grid.type(torch.FloatTensor).to(velocity.device)
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, src, flow):
        ...
        """
        vectors = [torch.arange(0, s) for s in flow.shape[2:]]
        grids = torch.meshgrid(vectors)
        grid = torch.stack(grids)  # y, x, z
        grid = torch.unsqueeze(grid, 0)  # add batch
...
class Lagrangian_flow(nn.Module):
    ...
    def forward(self, inf_flow, forward_flow=True):
        ...
        for k in range (1, seq_len):
            ...
            lag_flow[:, :, k] = sum_flow.squeeze(2) + torch.cat((lag_flow_x, lag_flow_y, lag_flow_z), dim=1)
...
class RViT(nn.Module):
    ...
    def __init__(self, image_size, patch_size, depth, length, heads, int_steps=7, mlp_dim=None, channels=2, dropout=0., emb_dropout=0, bidirection=True):
        ...
        num_patches = (image_size[0] * image_size[1] * image_size[2]) // (patch_size[0] * patch_size[1] * patch_size[2])
        ...
        dec_nf = [mlp_dim, 32, 32, 32, 16, 16]

        # One conv to get the flow field
        conv_fn = getattr(nn, 'Conv%dd' % ndims)
        self.flow_mean = conv_fn(dec_nf[-1], ndims, kernel_size=3, padding=1)
        self.flow_log_sigma = conv_fn(dec_nf[-1], ndims, kernel_size=3, padding=1)
        self.alpha, self.beta = 5, 1
        ...
        self.spatial_pos_embedding = nn.Parameter(torch.randn(1, num_patches, mlp_dim))
        self.temporal_pos_embedding = nn.Parameter(torch.randn(1, 128, mlp_dim))

        # Encoder functions
        self.enc = nn.ModuleList()
        for i in range(len(enc_nf)):
            prev_nf = 2 if i == 0 else enc_nf[i - 1]
            self.enc.append(conv_block(ndims, prev_nf, enc_nf[i], stride=2))
        
        self.middle_conv_block = nn.Conv3d(enc_nf[-1], enc_nf[-1], (1, 1, 1), (1, 1, 1))
        # self.embedding = nn.Conv3d(enc_nf[-1], enc_nf[-1] * 16, (patch_size, patch_size, 1), (patch_size, patch_size, 1))
        
        # Transformer Feature Encoding
        self.transformer = Encoder(mlp_dim, depth, heads, mlp_dim, bidirection, dropout_rate = dropout)

        # Velocity Filed Decoding
        self.dec = nn.ModuleList()
        self.dec.append(conv_block(ndims, enc_nf[-1], dec_nf[0]))  # 1
        self.dec.append(conv_block(ndims, dec_nf[0] * 2, dec_nf[1]))  # 2
        self.dec.append(conv_block(ndims, dec_nf[1] * 2, dec_nf[2]))  # 3
        self.dec.append(conv_block(ndims, dec_nf[2] + enc_nf[0], dec_nf[3]))  # 4
        self.dec.append(conv_block(ndims, dec_nf[3], dec_nf[4]))  # 5
        self.dec.append(conv_block(ndims, dec_nf[4] + 2, dec_nf[5], 1)) # Full size conv
        ...
        # Define Loss
        self.velocity_loss = VM_diffeo_loss(image_sigma=0.02, prior_lambda=10, flow_vol_shape=None)
        self.recon_loss = NCC(ndims=ndims)
    ...
    def forward(self, X, hidden, train=True):
        ...
        Z = rearrange(feature, 'b t (h w d) c  -> (b t) c h w d', h=p_h, w=p_w, d=d)

        # y =  + Z
        # y = torch.cat([self.middle_conv_block(Z), x_enc[-1]], dim=1)
        y = self.middle_conv_block(Z) + x_enc[-1]
        ...
        vts = rearrange(vts, '(b t) c h w d -> b t c h w d', b=b, t=t)
        ...
        for i in range(t):
            ...
            if train:
                recon_loss_tgt = self.recon_loss(tgt, y_src)
                # recon_loss_tgt_l1 = self.velocity_loss.recon_loss(tgt, y_src)
                recon_loss_tgt_l1 = 0
                recon_loss_src = self.recon_loss(src, y_tgt)
                # recon_loss_src_l1 = self.velocity_loss.recon_loss(src, y_tgt)
                recon_loss_src_l1 = 0
        ...
        if self.lag_regular:
            inf_flow_cross = torch.stack(inf_flow_all, dim=2)
            # Lagrangian flow
            lag_flow = self.lag_flow(inf_flow_cross)
            re_lag_flow = lag_flow
            lag_flow = rearrange(lag_flow, 'b c t h w d  -> (b t) c h w d')
            # Warp the reference frame by the Lagrangian flow
            src_start = src_X[:, 0:1]
            src_start_re = src_start.repeat(1, t, 1, 1, 1, 1)  # repeat the 1st frame to match other frames contained in a sequence
            src_start_re = rearrange(src_start_re, 'b t c h w d  -> (b t) c h w d')
            src_start_re = src_start_re.contiguous()

            lag_y_src = self.spatial_transform(src_start_re, lag_flow)

            neg_inf_flow_cross = torch.stack(neg_inf_flow_all, dim=2)
            # Neg Lagrangian flow
            neg_lag_flow = self.lag_flow(torch.flip(neg_inf_flow_cross, dims=[2]))
            re_neg_lag_flow = neg_lag_flow
            neg_lag_flow = rearrange(neg_lag_flow, 'b c t h w d  -> (b t) c h w d')
            # Warp the reference frame by the Lagrangian flow
            src_end = tgt_X[:, -1:]
            src_end_re = src_end.repeat(1, t, 1, 1, 1, 1)  # repeat the 1st frame to match other frames contained in a sequence
            src_end_re = rearrange(src_end_re, 'b t c h w d  -> (b t) c h w d')
            src_end_re = src_end_re.contiguous()

            lag_neg_y_src = self.spatial_transform(src_end_re, neg_lag_flow)
...
if __name__ == '__main__':
    ...
    print((input_size[0] * input_size[1] * input_size[2]) // (patch_size[0] * patch_size[1] * patch_size[2]))

    vid = torch.randn(1, length + 1, 1, input_size[0], input_size[1], input_size[2]).cuda(using_device)
    hidden = torch.zeros(1, (input_size[0] * input_size[1] * input_size[2]) // (patch_size[0] * patch_size[1] * patch_size[2]), mlp_dim).cuda(using_device)
    ...
    for name, param in v.named_parameters():
        ...
        for size_ in param.shape:
            mul *= size_                            # 统计每层参数个数
        sum_ += mul                                 # 累加每层参数个数
            # print('%14s : %s, %1.f' % (name, param.shape, mul))    # 打印参数名和参数数量
        # print('%s' % param)
    print('Params M:', sum_)
