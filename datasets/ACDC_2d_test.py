...
class ACDC_Dataset_Test(Dataset):
    def __init__(self, args, infos, crop_length=24, min_max=[-1, 1]):
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        data_start_frame_mask=sitk.GetArrayFromImage(data_start_frame_mask).astype(np.uint8)
        data_start_frame_mask=np.expand_dims(data_start_frame_mask, axis=0)[:, depth // 2, ...]

        data_end_frame_mask=sitk.ReadImage(case['anno_end_path'])
        data_end_frame_mask=sitk.GetArrayFromImage(data_end_frame_mask).astype(np.uint8)
        data_end_frame_mask=np.expand_dims(data_end_frame_mask, axis=0)[:, depth // 2, ...]

        current_video = data[:, depth // 2, ...]
        ...
        data_start_frame_mask = self.transform(np.expand_dims(data_start_frame_mask, axis=-1))
        data_end_frame_mask = self.transform(np.expand_dims(data_end_frame_mask, axis=-1))

        return current_video.transpose(0,1), data_start_frame_mask, data_end_frame_mask, case['ES_f']
...
if __name__ == '__main__':
    ...
    train_ds = ACDC_Dataset_Test(args, infos)
