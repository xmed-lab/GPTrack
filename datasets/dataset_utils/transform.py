...
def resize(clip, target_size, interpolation_mode):
    ...
    if interpolation_mode not in ['linear', 'bilinear', 'bicubic', 'trilinear']:
...
def to_tensor(clip):
    ...
    return clip.float().permute(0, 3, 1, 2)
...
class ResizedVideo(object):
    def __init__(self, size, p=1, interpolation_mode="bilinear"):
        if isinstance(size, numbers.Number):
            self.size = (int(size), int(size))
        else:
            self.size = size
...
class Rotate(object):
    ...
    """
    def __init__(self, factor: List[float] = (0, 180), p: float = 0.5) -> None:
        self.factor = factor
        self.p = p
    def __call__(self, video: torch.Tensor) -> torch.Tensor:
        if random.random() < self.p:
            ...
            return torchvision.transforms.functional.rotate(
                video, angle, interpolation=torchvision.transforms.functional.InterpolationMode.BILINEAR
            )
        else:
            return video
...
class Autocontrast(object):
    ...
    def __call__(self, video: torch.Tensor) -> torch.Tensor:
        if random.random() < self.p:
            video = torchvision.transforms.functional.autocontrast(video)
        return video
...
class Invert(object):
    ...
    def __call__(self, video: torch.Tensor) -> torch.Tensor:
        if random.random() < self.p:
            video = torchvision.transforms.functional.invert(video)
        
        return video


class Color_jitter(object):
    def __init__(self, factor: List[float] = (0, 2), p: float = 0.5) -> None:
        self.factor = factor
        self.p = p
    def __call__(self, video: torch.Tensor) -> torch.Tensor:
        if random.random() < self.p:
            video = self._adjust_saturation(video, random.uniform(self.factor[0], self.factor[1]))
        if random.random() < self.p:
            video = self._adjust_brightness(video, random.uniform(self.factor[0], self.factor[1]))
        if random.random() < self.p:
            video = self._adjust_sharpness(video, random.uniform(self.factor[0], self.factor[1]))
        
        return video
    
    def _adjust_saturation(self, video: torch.Tensor, factor: float, **kwargs) -> torch.Tensor:
        """
    ...
    def _adjust_brightness(self, video: torch.Tensor, factor: float, **kwargs) -> torch.Tensor:
        """
    ...
    def _adjust_sharpness(self, video: torch.Tensor, factor: float, **kwargs) -> torch.Tensor:
        """
