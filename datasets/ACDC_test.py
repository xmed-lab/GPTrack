...
class ACDC_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data_end_frame_mask=sitk.GetArrayFromImage(data_end_frame_mask).astype(np.float32)
        data_end_frame_mask=np.expand_dims(data_end_frame_mask, axis=0)
        ...
        if depth >= self.fineSize[1]:
            ...
            data_start_frame_mask = data_start_frame_mask[:, sd:sd + self.fineSize[1], ...]
            data_end_frame_mask = data_end_frame_mask[:, sd:sd + self.fineSize[1], ...]
        else:
            ...
            data_start_frame_mask_ = np.zeros([1, self.fineSize[1], height, width])
            data_start_frame_mask_[:, sd:sd + depth] = data_start_frame_mask
            data_start_frame_mask = data_start_frame_mask_

            data_end_frame_mask_ = np.zeros([1, self.fineSize[1], height, width])
        ...
        return current_video.permute(0, 2, 3, 1).unsqueeze(1), data_start_frame_mask.permute(0, 2, 3, 1), data_end_frame_mask.permute(0, 2, 3, 1), case['ES_f']
