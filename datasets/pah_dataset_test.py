...
class Seg_PAHDataset(Dataset):
    def __init__(self, args, infos, data_type=['middle', 'severe'], is_train=True, is_test=False, set_select=['gy'], view_num=['3'], is_video=True):
        self.args = args
        ...
        if is_train and not is_test:
            self.transform = transforms.Compose([
                                ...
                                transform.Rotate(p=0.6),
                                transform.Equalize(p=0.5),
                                ])
        else:
            self.transform = transforms.Compose([
                                transform.ToTensorVideo(),
                                ...
                                # transform.CenterCropVideo((448, 448)),
                                transform.ResizedVideo((128, 128)),
                                ])
    ...
    def __getitem__(self, index):
        ...
        index = index // 10 if self.is_train and not self.is_test else index
        if self.is_video:
            frame_list = list()
            ...
            if self.is_train:
                ...
                if max_sample_rate > 8:
                    sample_rate = random.randint(2, 8)
                elif 4 < max_sample_rate < 8:
                ...
                max_sample_rate = video_length // self.crop_length
                if max_sample_rate > 3:
                    sample_rate = 3
                elif max_sample_rate == 2:
                    sample_rate = 2
                else:
                    sample_rate = 1
            ...
            for frame in frame_list:
                ...
                # frame = frame.filter(ImageFilter.GaussianBlur(radius = radius_ratio))
            ...
            return video, float(mpap), float(pasp)
    ...
    def __len__(self):
        return len(self.data_list) * 10 if self.is_train and not self.is_test else len(self.data_list)
...
if __name__ == '__main__':
    ...
    train_ds = Seg_PAHDataset(args, infos, set_select=['gy','szfw','rmyy','shph'], view_num=['1','2','3','4'])
    train_loader = DataLoader(train_ds, batch_size=4, shuffle=True, num_workers=1)
    from einops import rearrange
    for img, mpap, pasp in tqdm(train_loader):
        print(img.shape)